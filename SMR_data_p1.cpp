...
#pragma omp parallel for
    ...
    void smr_ivw_gwas_gwas(char* outFileName, char* bFileName,char* gwasFileName, char* gwasFileName2, double maf,char* indilstName, char* snplstName,char* problstName,bool bFlag,double p_hetero,double ld_top,int m_hetero ,int opt_hetero, char* indilst2remove, char* snplst2exclde, char* problst2exclde,double p_smr, double p_y, char* refSNP, bool heidioffFlag, int cis_itvl,char* genelistName, int chr,int prbchr, char* prbname, char* fromprbname, char* toprbname,int prbWind,int fromprbkb, int toprbkb,bool prbwindFlag, char* genename,int snpchr, char* snprs, char* fromsnprs, char* tosnprs,int snpWind,int fromsnpkb, int tosnpkb,bool snpwindFlag,bool cis_flag,char* setlstName, char* geneAnnoFileName, int expanWind, double ld_min,double threshpsmrest, bool sampleoverlap, double pmecs, int minCor, double ld_top_multi,double afthresh,double percenthresh)
    {
        double theta=0;
        setNbThreads(thread_num);
        ...
        int snp_count=smr_setbased_test_ivw(&bdata, slctId, slct_bxz,slct_sexz,slct_pxz, slct_byz,slct_seyz, beta_ivw, se_ivw, p_ivw, h2cis, p_smr, p_y, ld_top_multi, snp4msmr, p_expo_provided);
        ...
        fclose(setlst);
        free_gwas_data( &gdata);
        free_gwas_data( &gdata2);
        
    }

    void smr_ivw_multipleSNP(char* outFileName, char* bFileName,char* gwasFileName, char* eqtlFileName, double maf,char* indilstName, char* snplstName,char* problstName,bool bFlag,double p_hetero,double ld_top,int m_hetero ,int opt_hetero, char* indilst2remove, char* snplst2exclde, char* problst2exclde,double p_smr, double p_y, char* refSNP, bool heidioffFlag, int cis_itvl,char* genelistName, int chr,int prbchr, char* prbname, char* fromprbname, char* toprbname,int prbWind,int fromprbkb, int toprbkb,bool prbwindFlag, char* genename,int snpchr, char* snprs, char* fromsnprs, char* tosnprs,int snpWind,int fromsnpkb, int tosnpkb,bool snpwindFlag,bool cis_flag,char* setlstName, char* geneAnnoFileName, int expanWind, double ld_min,double threshpsmrest, bool sampleoverlap, double pmecs, int minCor, double ld_top_multi,double afthresh,double percenthresh)
    {
        double theta=0;
        setNbThreads(thread_num);
        ...
        string outstr="probeID\tProbeChr\tGene\tProbe_bp\ttopSNP\ttopSNP_chr\ttopSNP_bp\tb_top\tse_top\tp_top\tb_ivw\tse_ivw\tp_ivw\tn_iv\th2cis\tp_HEIDI\tnsnp_HEIDI\n";
        if(fputs_checked(outstr.c_str(),smr))
        {
            printf("ERROR: in writing file %s .\n", smrfile.c_str());
            exit(EXIT_FAILURE);
        ...
        if(set_name.size()>0)
        {
            ...
            for(int i=0;i<probNum;i++)
            {
                ...
                outstr = probename + '\t' + atos(probechr) + '\t' + probegene + '\t' + atos(probebp) + '\t' + topsnpname + '\t' + atos(esdata._esi_chr[out_raw_id]) + '\t' + atos(esdata._esi_bp[out_raw_id]) + '\t';
...
void smr_rev_multipleSNP(char* outFileName, char* bFileName,char* gwasFileName, char* eqtlFileName, double maf,char* indilstName, char* snplstName,char* problstName,bool bFlag,double p_hetero,double ld_top,int m_hetero ,int opt_hetero, char* indilst2remove, char* snplst2exclde, char* problst2exclde,double p_gwas, double p_y, char* refSNP, bool heidioffFlag, int cis_itvl,char* genelistName, int chr,int prbchr, char* prbname, char* fromprbname, char* toprbname,int prbWind,int fromprbkb, int toprbkb,bool prbwindFlag, char* genename,int snpchr, char* snprs, char* fromsnprs, char* tosnprs,int snpWind,int fromsnpkb, int tosnpkb,bool snpwindFlag,bool cis_flag,char* setlstName, char* geneAnnoFileName, int expanWind, double ld_min,double threshpsmrest, bool sampleoverlap, double pmecs, int minCor, double ld_top_multi,double afthresh,double percenthresh)
    {
        double theta=0;
        setNbThreads(thread_num);
        ...
        for(int i=0;i<probNum;i++)
        {
            ...
            long maxid =fill_smr_wk_trans(&bdata, &gdata, &esdata, &smrwk, refSNP, heidioffFlag);
            if(refSNP!=NULL && maxid==-9) {
                printf("WARNING: can't find target SNP %s for probe %s.\n",refSNP, probename.c_str());
                continue;
            } //ref heidi SNP is not in selected SNPs
            ...
            // step2: get top-SNP
            printf("Checking the top-SNP in the region....\n");
            //Map<VectorXd> ei_byz(&smrwk.byz[0],smrwk.byz.size());
            //Map<VectorXd> ei_seyz(&smrwk.seyz[0],smrwk.seyz.size());
            //VectorXd zsyz;
            //zsyz=ei_byz.array()/ei_seyz.array();
            Map<VectorXd> ei_pyz(&smrwk.pyz[0],smrwk.pyz.size());
            VectorXd pyz=ei_pyz.array();
            if(refSNP==NULL) maxid=min_id(pyz); // now maxid point to the sig GWAS SNP or ref SNP in the new datastruct(not the raw).
            double pyz_val = pyz[maxid];
            
            string topsnpname=smrwk.rs[maxid];
            printf("The top SNP of probe %s is %s with p-value %e.\n", probename.c_str(), topsnpname.c_str(),pyz_val);
            if(refSNP==NULL && pyz_val> p_gwas){
                //printf("WARNING: no SNP passed the p-value threshold %e for Multiple-SNP SMR analysis for probe %s.\n", p_smr, probename.c_str());
                continue;
            } else {
                // printf("Conducting multi-SNP SMR and HEIDI test for probe %s...\n", probename.c_str());
            ...
            bool p_expo_provided = true;
            // reverse input
            int snp_count=smr_setbased_test_ivw(&bdata, slctId, slct_byz,slct_seyz,slct_pyz, slct_bxz,slct_sexz, beta_ivw, se_ivw, p_ivw, h2cis, p_gwas, p_y, ld_top_multi, snp4msmr, p_expo_provided);
            if(snp_count==-9) continue;
            // output snp set list
            string setstr=probename+'\n';
            if(fputs_checked(setstr.c_str(),setlst))
    ...
    int mr_mediation_analysis(bInfo* bdata, SMRWKM* smrwk, double &beta_xy, double &se_xy, double &beta_my, double &se_my, double &theta_T, double &se_t, double &beta_mx, double &se_mx, double &beta_mx_rev, double &se_mx_rev, double &theta_mt, double &se_mt, int &snp_count4mr, int &snp_count4mmr, int &snp_count4xmmr, int &snp_count4mxmr, double p_smr, double p_medsmr, double ld_top_multi, vector<string> &snp4msmr, bool multi_corr_snp_flg, bool multi_exp_priority_flg, bool get_snp_effects_flg, int min_snp, FILE* snpfile, string snpfilename, string exponame, string medname)
    {
        string logstr;
        string snpstr;
        logstr="Conducting mediation MR analysis...";
        cout<<logstr<<endl;
        ...
        // Id4mmr includes SNPs for MR of mediator to outcome 
        // Id4xmmr includes SNPs for MR of exposure to mediator
        // Id4mxmr includes SNPs for MR of mediator to exposure
        
        vector<uint32_t> Id4multiall, Id4multix, Id4multim, Id4mr, Id4mmr, Id4xmmr, Id4mxmr;

        vector<double> bxz4multiall, bxz4mr, bxz4mmr, bxz4xmmr, bxz4mxmr;
        vector<double> sexz4multiall, sexz4mr, sexz4mmr, sexz4xmmr, sexz4mxmr;
        vector<double> bmz4multiall, bmz4mr, bmz4mmr, bmz4xmmr, bmz4mxmr;
        vector<double> semz4multiall, semz4mr, semz4mmr, semz4xmmr, semz4mxmr;
        vector<double> byz4multiall, byz4mr, byz4mmr, seyz4multiall, seyz4mr, seyz4mmr;
        vector<double> zxz4multix, zmz4multim, zxz4mr, zxz4mmr, zxz4xmmr, zmz4mr, zmz4mmr, zmz4mxmr, zyz4mr, zyz4mmr;
        vector<string> snpnameall, allele1all, allele2all;
        ...
        for(int j=0;j<smrwk->curId.size();j++)
        {
            ...
            /*
            if ((fabs(smrwk->bxz[j]) > fabs(smrwk->byz[j])) && (fabs(smrwk->bmz[j]) > fabs(smrwk->byz[j]))){
                if((ztmpx>=z_x) || (ztmpm>=z_m)){
            ...
            if (((fabs(smrwk->bxz[j]) > fabs(smrwk->byz[j])) && (ztmpx>=z_x)) || ((fabs(smrwk->bmz[j]) > fabs(smrwk->byz[j])) && (ztmpm>=z_m))){
            ...
            if ((ztmpx>=z_x)  && (fabs(smrwk->bxz[j]) > fabs(smrwk->byz[j]))){
            ...
            if ((ztmpm>=z_m) && (fabs(smrwk->bmz[j]) > fabs(smrwk->byz[j]))){
            Id4mmr.push_back(smrwk->curId[j]);
            bxz4mmr.push_back(smrwk->bxz[j]);
            sexz4mmr.push_back(smrwk->sexz[j]);
            bmz4mmr.push_back(smrwk->bmz[j]);
            semz4mmr.push_back(smrwk->semz[j]);
            zmz4mmr.push_back(smrwk->bmz[j]/smrwk->semz[j]);
            byz4mmr.push_back(smrwk->byz[j]);
            seyz4mmr.push_back(smrwk->seyz[j]);
            zyz4mmr.push_back(smrwk->byz[j]/smrwk->seyz[j]);

            }

            if ((ztmpx>=z_x) && (fabs(smrwk->bxz[j]) > fabs(smrwk->bmz[j]))){
            ...
            if ((ztmpm>=z_m) && (fabs(smrwk->bmz[j]) > fabs(smrwk->bxz[j]))){
                Id4mxmr.push_back(smrwk->curId[j]);
                bxz4mxmr.push_back(smrwk->bxz[j]);
                bmz4mxmr.push_back(smrwk->bmz[j]);
                zmz4mxmr.push_back(smrwk->bmz[j]/smrwk->semz[j]);
                sexz4mxmr.push_back(smrwk->sexz[j]);
                semz4mxmr.push_back(smrwk->semz[j]);
            }
        ...
        snp_count4mr=(int)Id4mr.size();
        snp_count4mmr=(int)Id4mmr.size();
        snp_count4xmmr=(int)Id4xmmr.size();
        snp_count4mxmr=(int)Id4mxmr.size();

        logstr=atos(Id4multiall.size()) + " SNPs with stronger exposure and mediator than direct effects passed the exposure p-value " + atos(p_smr) + " or mediator p-value " + atos(p_medsmr) + ".";
        cout<<logstr<<endl;
        logstr=atos(Id4mr.size()) + " SNPs with stronger exposure than direct effects passed the exposure p-value threshold " + atos(p_smr) + ".";
        cout<<logstr<<endl;
        //printf("%ld SNPs with stronger exposure and mediator than direct effects passed the exposure p-value %6.2e or mediator p-value %6.2e.\n",Id4multiall.size(), p_smr, p_medsmr);
        //printf("%ld SNPs with stronger exposure than direct effects passed the exposure p-value threshold %6.2e.\n",Id4mr.size(), p_smr);
        if((snp_count4multi<3) || (snp_count4mr<1)) return -9; // we need at least 3 SNPs to calculate a direct effect
        /* step5: Pruning */
    
        // SNPs for multivariable regression, perform pruning once on exposure-associated SNPs and once on mediator-associated SNPs
          
        vector<int> sub_indx4multi, sub_indx4multix, sub_indx4multim;
        MatrixXd _X4multi, _X4multix, _X4multim;
        make_XMat(bdata, Id4multix, _X4multix); //_X: one row one individual, one column one SNP 
        make_XMat(bdata, Id4multim, _X4multim); 
        ...
        sbat_calcu_lambda(_X4multim, snp_count4multim,  sbat_ld_cutoff, sub_indx4multim, zmz4multim); //the index of slectId, snp_count can change here, pruning is based on eQTL p-values (zyz4smr is not used)
        if((snp_count4multix + snp_count4multim)<3) return -9;

        // combine the two SNP sets 
        vector<uint32_t> Id4multi, Id4multim_pruned, Id4multix_pruned;
        for (int j = 0; j < sub_indx4multix.size(); j++) Id4multix_pruned.push_back(Id4multix[sub_indx4multix[j]]);
        for (int j = 0; j < sub_indx4multim.size(); j++) Id4multim_pruned.push_back(Id4multim[sub_indx4multim[j]]);
        ...
        bool exp_snp, med_snp;

        for(int j=0; j<Id4multiall.size();j++)
        {
            exp_snp = find(Id4multix_pruned.begin(), Id4multix_pruned.end(), Id4multiall[j]) != Id4multix_pruned.end();
            med_snp = find(Id4multim_pruned.begin(), Id4multim_pruned.end(), Id4multiall[j]) != Id4multim_pruned.end();
            ...
            if (exp_snp || med_snp){               
                ...
                if (exp_snp && multi_exp_priority_flg){
                    pruning_weights.push_back(1);
                }
                else if (med_snp && multi_exp_priority_flg){
                    pruning_weights.push_back(0);
                }
                else if (exp_snp && !multi_exp_priority_flg){
                    pruning_weights.push_back(0);
                }
                else if (med_snp && !multi_exp_priority_flg){
                    pruning_weights.push_back(1);
        ...
        if((snp_count4multi<3)) return -9; // we need at least 3 SNPs to calculate a direct effect

        // Prune at 0.9 to avoid multi-collinearity issues with C matrix
        double ld_col;
        if (multi_corr_snp_flg){
            ld_col = sqrt(0.9);
        } else {
            ld_col = sqrt(ld_top_multi);
        }
        make_XMat(bdata, Id4multi, _X4multi);
        sbat_calcu_lambda(_X4multi, snp_count4multi, ld_col, sub_indx4multi, pruning_weights);
        if((snp_count4multi)<3) return -9;
        ...
        cout<<logstr<<endl;
        //printf("%ld SNPs passed LD-square threshold of %6.2f and %ld SNPs are excluded in the multivariable effect calculation.\n",sub_indx4multi.size(), ld_top_multi, Id4multiall.size()-sub_indx4multi.size());
        // IVW calculation
        logstr=atos(sub_indx4multi.size()) + " SNPs are included in the multivariable effect calculation.";
        cout<<logstr<<endl;
        //printf("%ld SNPs are included in the multivariable effect calculation.\n",sub_indx4multi.size());
        ...
        sbat_calcu_lambda(_X4mr, snp_count4mr,  sbat_ld_cutoff, sub_indx4mr, zxz4mr); //the index of slectId, snp_count can change here, pruning is based on eQTL p-values (zyz4smr is not used)
        ...
        cout<<logstr<<endl;
        //printf("%ld SNPs passed LD-square threshold of %6.2f and %ld SNPs are excluded in the MR total effect calculation.\n",sub_indx4mr.size(), ld_top_multi, Id4mr.size()-sub_indx4mr.size());
        if(snp_count4mr<1) return -9;
        logstr=atos(sub_indx4mr.size()) + " SNPs are included in the total effect calculation.";
        cout<<logstr<<endl;
        //printf("%ld SNPs are included in the total effect calculation.\n",sub_indx4mr.size());

        // SNPs for MR total mediator effect

        vector<int> sub_indx4mmr;
        if (snp_count4mmr > 0)
        {
            MatrixXd _X4mmr;
            make_XMat(bdata,Id4mmr, _X4mmr); //_X: one row one individual, one column one SNP
            sbat_calcu_lambda(_X4mmr, snp_count4mmr, sbat_ld_cutoff, sub_indx4mmr, zmz4mmr); //the index of slectId, snp_count can change here, pruning is based on eQTL p-values (zyz4smr is not used)
        }
        ...
        if (snp_count4xmmr > 0)
        {
            ...
            sbat_calcu_lambda(_X4xmmr, snp_count4xmmr, sbat_ld_cutoff, sub_indx4xmmr, zxz4xmmr); //the index of slectId, snp_count can change here, pruning is based on eQTL p-values (zyz4smr is not used)
        }

        // SNPs for MR mediator to exposure
        vector<int> sub_indx4mxmr;
        if (snp_count4mxmr > 0)
        {
            MatrixXd _X4mxmr;
            make_XMat(bdata,Id4mxmr, _X4mxmr); //_X: one row one individual, one column one SNP
            sbat_calcu_lambda(_X4mxmr, snp_count4mxmr, sbat_ld_cutoff, sub_indx4mxmr, zmz4mxmr); //the index of slectId, snp_count can change here, pruning is based on eQTL p-values (zyz4smr is not used)
        }
        ...
        VectorXd yz(snp_count4multi);
        MatrixXd M(2,2), df_dg(2, snp_count4multi), df_dG(2, 2*snp_count4multi), J(2, 3*snp_count4multi), vars(2,2);
        VectorXd SEs(3*snp_count4multi);
        MatrixXd Sigma(3*snp_count4multi, 3*snp_count4multi), I_mat, R;
        I_mat.setIdentity(snp_count4multi, snp_count4multi); 
        R.setIdentity(3,3); 
        Vector2d betas;
        ...
        MatrixXd C_r, C(snp_count4multi,snp_count4multi);
        cor_calc(C_r, _X4multi);
        C = C_r.array()*C_r.array();
        C = (C.array() < 1e-5).select(0, C); 
        MatrixXd W(snp_count4multi, snp_count4multi); // weight matrix (inverse C matrix)
        ...
        betas = M*(X.transpose()*W*yz);
        beta_xy = betas(0);
        beta_my = betas(1);

        // Calculate the standard errors
        df_dg = M*X.transpose()*W;
        df_dG = kroneckerProduct(M, yz.transpose()*W*((X*M*X.transpose())*W + I_mat)) + kroneckerProduct(-yz.transpose()*W*X*M, M*X.transpose()*W);

        J << df_dG, df_dg; 
        Sigma = (SEs*SEs.transpose()).array() * kroneckerProduct(C, R).array();

        vars = J*Sigma*J.transpose();
        se_xy = sqrt(vars(0,0));
        se_my = sqrt(vars(1,1));

        /*MatrixXd X(sub_indx4direct.size(), 2);
        Vector2d betas, vars;
        double sigma_2;

        yz.resize(sub_indx4direct.size());
        res.resize(sub_indx4direct.size());

        for(int j=0; j<sub_indx4direct.size();j++)
        {
            yz(j) = byz4direct[sub_indx4direct[j]];
            X(j,0) = bxz4direct[sub_indx4direct[j]];
            X(j,1) = bmz4direct[sub_indx4direct[j]];
        }

        betas = X.bdcSvd(ComputeThinU | ComputeThinV).solve(yz);
        beta_xy = betas(0);
        beta_my = betas(1);

        res = (yz - (X*betas));
        sigma_2 = res.dot(res)/(snp_count4direct - 2);
        vars = (sigma_2*(X.transpose()*X).inverse()).diagonal();
        se_xy = sqrt(vars[0]);
        se_my = sqrt(vars[1]); */

        //Calculate the total effect 
        VectorXd xz, res, seyz;
        double xtx_inv, sigma_2;
        //VectorXd bxy4smr, varxy4smr, weights, one;
        //double var_ivw, z_ivw;

        yz.resize(0);
        yz.resize(sub_indx4mr.size());
        res.resize(sub_indx4mr.size());
        xz.resize(sub_indx4mr.size());
        seyz.resize(sub_indx4mr.size());

        //bxy4smr.resize(sub_indx4mr.size());
        //varxy4smr.resize(sub_indx4mr.size());
        //weights.resize(sub_indx4mr.size());
        //one.resize(sub_indx4mr.size());

        for(int j=0; j<sub_indx4mr.size();j++)
        {
            yz(j) = byz4mr[sub_indx4mr[j]];
            xz(j) = bxz4mr[sub_indx4mr[j]];
            seyz(j) = seyz4mr[sub_indx4mr[j]];
        }

        // Calculate the total effect
        xtx_inv = 1/(xz.dot(xz));
        theta_T = xtx_inv*(xz.dot(yz));
        //res = (yz - xz*theta_T);
        //sigma_2 = res.dot(res)/(snp_count4mr-1);
        sigma_2 = seyz.dot(seyz)/snp_count4mr;
        se_t = sqrt(sigma_2*xtx_inv);

        /* Calculate the classical IVW estimate

        weights = one.array()/varxy4smr.array();
        var_ivw = 1/weights.sum();
        se_ivw = sqrt(var_ivw);
        beta_ivw = (bxy4smr.array()*weights.array()).sum()*var_ivw;
        z_ivw = beta_ivw/se_ivw;
        p_ivw = 2*pnorm(fabs(z_ivw));
        */

        //Calculate the total mediator to outcome effect

        VectorXd mz;

        if (snp_count4mmr > 0) {
            yz.resize(0);
            yz.resize(sub_indx4mmr.size());
            res.resize(0);
            res.resize(sub_indx4mmr.size());
            mz.resize(0);
            mz.resize(sub_indx4mmr.size());
            seyz.resize(0);
            seyz.resize(sub_indx4mmr.size());
            
            for(int j=0; j<sub_indx4mmr.size();j++)
            {
                yz(j) = byz4mmr[sub_indx4mmr[j]];
                mz(j) = bmz4mmr[sub_indx4mmr[j]];
                seyz(j) = seyz4mmr[sub_indx4mmr[j]];
            }

            // total mediator to outcome effect

            xtx_inv = 1/(mz.dot(mz));
            theta_mt = xtx_inv*(mz.dot(yz));
            //res = (yz - mz*theta_mt);
            //sigma_2 = res.dot(res)/(snp_count4mmr-1);
            sigma_2 = seyz.dot(seyz)/snp_count4mmr;
            se_mt = sqrt(sigma_2*xtx_inv);
        ...
        if (snp_count4xmmr > 0) {
            xz.resize(0);
            xz.resize(sub_indx4xmmr.size());
            res.resize(0);
            res.resize(sub_indx4xmmr.size());
            mz.resize(0);
            mz.resize(sub_indx4xmmr.size());
            semz.resize(sub_indx4xmmr.size());
            
            for(int j=0; j<sub_indx4xmmr.size();j++)
            {
                xz(j) = bxz4xmmr[sub_indx4xmmr[j]];
                mz(j) = bmz4xmmr[sub_indx4xmmr[j]];
                semz(j) = semz4xmmr[sub_indx4xmmr[j]];
            }

            // total mediator to outcome effect

            xtx_inv = 1/(xz.dot(xz));
            beta_mx = xtx_inv*(xz.dot(mz));
            //res = (mz - xz*beta_mx);
            //sigma_2 = res.dot(res)/(snp_count4xmmr-1);
            sigma_2 = semz.dot(semz)/snp_count4xmmr;
            se_mx = sqrt(sigma_2*xtx_inv);
        }

        // Calculate the mediator to exposure effect

        VectorXd bmx4smr, varmx4smr, weights, one;
        double var_ivw, z_ivw;

        /*
        if (snp_count4mxmr > 2) {
            
            bmx4smr.resize(sub_indx4mxmr.size());
            varmx4smr.resize(sub_indx4mxmr.size());
            weights.resize(sub_indx4mxmr.size());
            one.resize(sub_indx4mxmr.size());
            
            for(int j=0; j<sub_indx4mxmr.size();j++)
            {
                bmx4smr[j] = bxz4mxmr[sub_indx4mxmr[j]]/bmz4mxmr[sub_indx4mxmr[j]];
                varmx4smr[j] = pow(sexz4mxmr[sub_indx4mxmr[j]], 2)/pow(bmz4mxmr[sub_indx4mxmr[j]],2) + pow(bxz4mxmr[sub_indx4mxmr[j]]*semz4mxmr[sub_indx4mxmr[j]],2)/pow(bmz4mxmr[sub_indx4mxmr[j]],4);
                one[j] = 1;
            }

            weights = one.array()/varmx4smr.array();
            var_ivw = 1/weights.sum();
            se_mx_rev = sqrt(var_ivw);
            beta_mx_rev = (bmx4smr.array()*weights.array()).sum()*var_ivw;
            
        } */

        if (snp_count4mxmr > 0) {
            xz.resize(0);
            xz.resize(sub_indx4mxmr.size());
            res.resize(0);
            res.resize(sub_indx4mxmr.size());
            mz.resize(0);
            mz.resize(sub_indx4mxmr.size());
            sexz.resize(0);
            sexz.resize(sub_indx4mxmr.size());
            
            for(int j=0; j<sub_indx4mxmr.size();j++)
            {
                xz(j) = bxz4mxmr[sub_indx4mxmr[j]];
                mz(j) = bmz4mxmr[sub_indx4mxmr[j]];
                sexz(j) = sexz4mxmr[sub_indx4mxmr[j]];
            }

            xtx_inv = 1/(mz.dot(mz));
            beta_mx_rev = xtx_inv*(mz.dot(xz));
            //res = (xz - mz*beta_mx_rev);
            //sigma_2 = res.dot(res)/(snp_count4mxmr-1);
            sigma_2 = sexz.dot(sexz)/snp_count4mxmr;
            se_mx_rev = sqrt(sigma_2*xtx_inv);
        } 
    ...
    void smr_e2e_mediation(char* outFileName, char* bFileName,char* eqtlFileName, char* eqtlFileName2, char* gwasFileName, double maf,char* indilstName, char* snplstName,char* problstName, char* oproblstName,char* eproblstName,bool bFlag, double ld_top,  char* indilst2remove, char* snplst2exclde, char* problst2exclde, char* oproblst2exclde, char* eproblst2exclde, double p_smr, double p_medsmr, int cis_itvl,char* traitlstName, char* oprobe, char* eprobe, char* oprobe2rm, char* eprobe2rm, double threshpsmrest, bool opt, double ld_min, bool cis2all, bool sampleoverlap, double pmecs, int minCor,double ld_top_multi, char* targetsnpproblstName, char* snpproblstName,double afthresh,double percenthresh, bool multi_corr_snp_flg, bool multi_exp_priority_flg, bool get_snp_effects_flg, int min_snp)
    {
        ...
        outstr="Chr\tExpo_ID\tExpo_Gene\tExpo_bp\tMed_ID\tMed_Gene\ttheta_T\tse_t\ttheta_dA\tse_dA\ttheta_iA\tse_iA\ttheta_dB\tse_dB\ttheta_iB\tse_iB\ttheta_mt\tse_mt\tb_xy\tse_xy\tb_my\tse_my\tb_mx\tse_mx\tb_mx_rev\tse_mx_rev\tn_snps_multi\tn_snps_mr\tn_snps_mmr\tn_snps_xmmr\tn_snps_mxmr\n";
        if(fputs_checked(outstr.c_str(),smr))
        {
            printf("ERROR: in writing file %s .\n", smrfile.c_str());
            exit(EXIT_FAILURE);
        ...
        for( int ii=0;ii<probNum;ii++){
            ...
            // find the matching probe to pqtl data
            int jj=-9;
            string exponame;
            string expogene;
            ...
            if (jj==-9){
                //printf("No matching exposure probe found for mediator [ %s ].\n", medname.c_str());
            ...
            cout<< "The top SNP of exposure probe [ " + medname + " ] is " + topsnpnamem + " with p-value " + atos(pmz_val) <<endl;
            //printf("The top SNP of exposure probe [ %s ] is %s with p-value %e.\n", exponame.c_str(), topsnpnamex.c_str(),pxz_val);
            //printf("The top SNP of mediation probe [ %s ] is %s with p-value %e.\n", medname.c_str(), topsnpnamem.c_str(),pmz_val);
            ...
            vector<string> snp4msmr;
            double theta_T = -9, se_t = -9, beta_xy = -9, se_xy = -9, beta_my = -9, se_my = -9, beta_mx = -9, se_mx = -9, beta_mx_rev = -9, se_mx_rev = -9; 
            double theta_mt = -9, se_mt = -9;
            int snp_count4mr, snp_count4mmr, snp_count4xmmr, snp_count4mxmr;

            int snp_count=mr_mediation_analysis(&bdata, &smrwk, beta_xy, se_xy, beta_my, se_my, theta_T, se_t, beta_mx, se_mx, beta_mx_rev, se_mx_rev, theta_mt, se_mt, snp_count4mr, snp_count4mmr, snp_count4xmmr, snp_count4mxmr, p_smr, p_medsmr, ld_top_multi, snp4msmr, multi_corr_snp_flg, multi_exp_priority_flg, get_snp_effects_flg, min_snp, snpfile, snpfilename, exponame, medname);
            if(snp_count==-9){
                //printf("No mediation analysis performed because less than 3 SNPs passed the p-value or LD-square thresholds. \n");
                logstr="No mediation analysis performed because less than 3 SNPs passed the p-value or LD-square thresholds.";
                cout<<logstr<<endl;
                continue;
            }

            // calculate direct and indirect effects/se with method A and B
            double theta_dA, theta_dB, theta_iA, theta_iB, se_dA, se_dB, se_iA, se_iB;

            theta_dA = beta_xy;
            se_dA = se_xy;
            theta_iA = theta_T - theta_dA;
            se_iA = sqrt(se_t*se_t + se_dA*se_dA);

            theta_iB = beta_mx*beta_my;
            se_iB = sqrt(se_mx*se_mx*se_my*se_my + se_mx*se_mx*beta_my*beta_my + se_my*se_my*beta_mx*beta_mx);
            theta_dB = theta_T - theta_iB;
            se_dB = sqrt(se_t*se_t + se_iB*se_iB);
            ...
            outstr =  atos(medchr) + '\t' + exponame + '\t' + expogene + '\t' + atos(expobp) + '\t' + medname + '\t' + medgene + '\t'; 
            outstr += atos(theta_T)  + '\t' + atos(se_t) + '\t' + atos(theta_dA) + '\t' + atos(se_dA) + '\t' + atos(theta_iA) + '\t' + atos(se_iA) +'\t';
            outstr += atos(theta_dB) + '\t' + atos(se_dB) +'\t' + atos(theta_iB) + '\t' + atos(se_iB) +'\t' + atos(theta_mt) + '\t' + atos(se_mt) +'\t';
            outstr += atos(beta_xy)  + '\t' + atos(se_xy) + '\t' + atos(beta_my) + '\t' + atos(se_my) + '\t';
            outstr += atos(beta_mx) + '\t' + atos(se_mx) + '\t' + atos(beta_mx_rev) + '\t' + atos(se_mx_rev) + '\t';
            outstr += atos(snp_count) + '\t' + atos(snp_count4mr) + '\t' + atos(snp_count4mmr)+ '\t' + atos(snp_count4xmmr)+ '\t' + atos(snp_count4mxmr)+ '\n';
    ...
    int mr_mediation_pruning(bInfo* bdata, SMRWKM* smrwk, int expchr, int medchr, double pxz_val, double pmz_val, double p_smr, double p_medsmr, double ld_top_multi, bool multi_corr_snp_flg, bool multi_exp_priority_flg, bool get_snp_effects_flg, int min_snp, FILE* snpfile, string snpfilename, string exponame, string medname)
    {
        string logstr;
        string snpstr;
        logstr="Conducting pruning...";
        cout<<logstr<<endl;
        ...
        vector<uint32_t> Id4multiall, Id4multiallx, Id4multiallm, Id4multix, Id4multim;

        vector<double> bxz4multiall, bxz4multix, bxz4multim;
        vector<double> sexz4multiall, sexz4multix, sexz4multim;
        vector<double> bmz4multiall, bmz4multix, bmz4multim;
        vector<double> semz4multiall, semz4multix, semz4multim;
        vector<double> byz4multiall, byz4multix, byz4multim;
        vector<double> seyz4multiall, seyz4multix, seyz4multim;
        vector<double> zxz4multiallx, zmz4multiallm, zxz4multix, zmz4multim;
        vector<string> snpnameall, allele1all, allele2all, snpnamex, allele1x, allele2x, snpnamem, allele1m, allele2m;

        double z_x= fabs(qnorm5(p_smr/2));
        double z_m= fabs(qnorm5(p_medsmr/2));

        string pruning_mode = "exposure_only";
        ...
        for(int j=0;j<smrwk->curId.size();j++)
        {
            ...
            if ((expchr == medchr) && (pmz_val < p_medsmr)){

                pruning_mode = "exposure_mediator_together";
                ...
                if (((fabs(smrwk->bxz[j]) > fabs(smrwk->byz[j])) && (ztmpx>=z_x)) || ((fabs(smrwk->bmz[j]) > fabs(smrwk->byz[j])) && (ztmpm>=z_m))){
                    ...
                    if(ztmpx>=z_x){
                        Id4multiallx.push_back(smrwk->curId[j]);
                        zxz4multiallx.push_back(smrwk->bxz[j]/smrwk->sexz[j]);
                    }

                    if(ztmpm>=z_m){
                        Id4multiallm.push_back(smrwk->curId[j]);
                        zmz4multiallm.push_back(smrwk->bmz[j]/smrwk->semz[j]);
                    }
                }
            } else if ((expchr != medchr) && (pmz_val < p_medsmr)){

                pruning_mode = "exposure_mediator_separate";
                if ((fabs(smrwk->bxz[j]) > fabs(smrwk->byz[j])) && (ztmpx>=z_x)){

                    Id4multix.push_back(smrwk->curId[j]);
                    bxz4multix.push_back(smrwk->bxz[j]);
                    bmz4multix.push_back(smrwk->bmz[j]);
                    byz4multix.push_back(smrwk->byz[j]);
                    sexz4multix.push_back(smrwk->sexz[j]);
                    semz4multix.push_back(smrwk->semz[j]);
                    seyz4multix.push_back(smrwk->seyz[j]);
                    zxz4multix.push_back(smrwk->bxz[j]/smrwk->sexz[j]);
                    snpnamex.push_back(smrwk->rs[j]);
                    allele1x.push_back(smrwk->allele1[j]);
                    allele2x.push_back(smrwk->allele2[j]);

                } else if ((fabs(smrwk->bmz[j]) > fabs(smrwk->byz[j])) && (ztmpm>=z_m)){

                    Id4multim.push_back(smrwk->curId[j]);
                    bxz4multim.push_back(smrwk->bxz[j]);
                    bmz4multim.push_back(smrwk->bmz[j]);
                    byz4multim.push_back(smrwk->byz[j]);
                    sexz4multim.push_back(smrwk->sexz[j]);
                    semz4multim.push_back(smrwk->semz[j]);
                    seyz4multim.push_back(smrwk->seyz[j]);
                    zmz4multim.push_back(smrwk->bmz[j]/smrwk->semz[j]);
                    snpnamem.push_back(smrwk->rs[j]);
                    allele1m.push_back(smrwk->allele1[j]);
                    allele2m.push_back(smrwk->allele2[j]);
        ...
        int snp_count4multix, snp_count4multim;
        int snp_count4multi;
        ...
        vector<double> bxz, bmz, byz, sexz, semz, seyz;
        vector<string> snpname, allele1, allele2;

        bool exp_snp, med_snp;
        double ld_col;

        if (pruning_mode == "exposure_mediator_together"){

            snp_count4multix= (int)Id4multiallx.size();
            snp_count4multim = (int)Id4multiallm.size();
            snp_count4multi= snp_count4multix + snp_count4multim;
            if (snp_count4multi < min_snp) return -9;

            make_XMat(bdata, Id4multiallx, _X4multix); //_X: one row one individual, one column one SNP 
            make_XMat(bdata, Id4multiallm, _X4multim); 

            sbat_calcu_lambda(_X4multix, snp_count4multix,  sbat_ld_cutoff, sub_indx4multix, zxz4multiallx);
            sbat_calcu_lambda(_X4multim, snp_count4multim,  sbat_ld_cutoff, sub_indx4multim, zmz4multiallm); //the index of slectId, snp_count can change here, pruning is based on eQTL p-values (zyz4smr is not used)
            
            if((snp_count4multix + snp_count4multim)< min_snp) return -9;

            // combine the two SNP sets 
            
            for (int j = 0; j < sub_indx4multix.size(); j++) Id4multix_pruned.push_back(Id4multiallx[sub_indx4multix[j]]);
            for (int j = 0; j < sub_indx4multim.size(); j++) Id4multim_pruned.push_back(Id4multiallm[sub_indx4multim[j]]);
            ...
            for(int j=0; j<sub_indx4multi.size();j++){
                ...
                bxz.push_back(bxz4multi[sub_indx4multi[j]]);
                sexz.push_back(sexz4multi[sub_indx4multi[j]]);
                bmz.push_back(bmz4multi[sub_indx4multi[j]]);
                semz.push_back(semz4multi[sub_indx4multi[j]]);
                byz.push_back(byz4multi[sub_indx4multi[j]]);
                seyz.push_back(seyz4multi[sub_indx4multi[j]]);

                snpname.push_back(snpnamemulti[sub_indx4multi[j]]);
                allele1.push_back(allele1multi[sub_indx4multi[j]]);
                allele2.push_back(allele2multi[sub_indx4multi[j]]);
      
            }
        } else if (pruning_mode == "exposure_mediator_separate"){

            snp_count4multix = (int)Id4multix.size();
            snp_count4multim = (int)Id4multim.size();
            snp_count4multi = snp_count4multix + snp_count4multim;
            ...
            for(int j=0; j<sub_indx4multix.size();j++){

                bxz.push_back(bxz4multix[sub_indx4multix[j]]);
                sexz.push_back(sexz4multix[sub_indx4multix[j]]);
                bmz.push_back(bmz4multix[sub_indx4multix[j]]);
                semz.push_back(semz4multix[sub_indx4multix[j]]);
                byz.push_back(byz4multix[sub_indx4multix[j]]);
                seyz.push_back(seyz4multix[sub_indx4multix[j]]);

                snpname.push_back(snpnamex[sub_indx4multix[j]]);
                allele1.push_back(allele1x[sub_indx4multix[j]]);
                allele2.push_back(allele2x[sub_indx4multix[j]]);
      
            }

            for(int j=0; j<sub_indx4multim.size();j++){

                bxz.push_back(bxz4multim[sub_indx4multim[j]]);
                sexz.push_back(sexz4multim[sub_indx4multim[j]]);
                bmz.push_back(bmz4multim[sub_indx4multim[j]]);
                semz.push_back(semz4multim[sub_indx4multim[j]]);
                byz.push_back(byz4multim[sub_indx4multim[j]]);
                seyz.push_back(seyz4multim[sub_indx4multim[j]]);

                snpname.push_back(snpnamem[sub_indx4multim[j]]);
                allele1.push_back(allele1m[sub_indx4multim[j]]);
                allele2.push_back(allele2m[sub_indx4multim[j]]);
      
            }

        } else if (pruning_mode == "exposure_only"){
            snp_count4multi = (int)Id4multix.size();

            if (snp_count4multi < min_snp) return -9;

            make_XMat(bdata, Id4multix, _X4multix); 
            sbat_calcu_lambda(_X4multix, snp_count4multi,  sbat_ld_cutoff, sub_indx4multix, zxz4multix);
            if (snp_count4multi < min_snp) return -9;
        ...
        if (get_snp_effects_flg){
            ...
            for (int j=0; j<snp_count4multi; j++){
                snpstr = exponame + '\t' + medname + '\t' + snpname[j] + '\t' + allele1[j] + '\t' + allele2[j] + '\t';
                snpstr += atos(bxz[j]) + '\t' + atos(sexz[j]) + '\t';
                snpstr +=  atos(bmz[j]) + '\t' + atos(semz[j]) + '\t';
                snpstr += atos(byz[j]) + '\t' + atos(seyz[j]) + '\n'; 
            ...
            logstr= atos(snp_count4multi)+ " SNPs for the exposure [ " + exponame + " ] - mediator [ " + medname + " ] pair were written down.\n";
            cout<<logstr<<endl;
        }
    ...
    void smr_e2multiple_mediation(char* outFileName, char* bFileName,char* eqtlFileName, char* eqtlFileName2, char* gwasFileName, double maf,char* indilstName, char* snplstName,char* problstName, char* oproblstName,char* eproblstName,bool bFlag, double ld_top,  char* indilst2remove, char* snplst2exclde, char* problst2exclde, char* oproblst2exclde, char* eproblst2exclde, double p_smr, double p_medsmr, int cis_itvl,char* traitlstName, char* oprobe, char* eprobe, char* oprobe2rm, char* eprobe2rm, double threshpsmrest, bool opt, double ld_min, bool cis2all, bool sampleoverlap, double pmecs, int minCor,double ld_top_multi, char* targetsnpproblstName, char* snpproblstName,double afthresh,double percenthresh, bool multi_corr_snp_flg, bool multi_exp_priority_flg, bool get_snp_effects_flg, int min_snp)
    {
        ...
        unsigned int probNum = mdata._probNum;
        unsigned int expNum = includebk.size();

        logstr= atos(probNum)+ " mediation probes are included in the analysis.\n";
        cout<<logstr<<endl;

        logstr= atos(expNum)+ " exposure probes are included in the analysis.\n";
        cout<<logstr<<endl;

        float progr0=0.0 , progr1;
        progress_print(progr0);

        for( int ej=0; ej < expNum; ej++){
            ...
            for( int ii=0;ii<probNum;ii++){
                ...
                progr1=1.0*ii/(probNum*expNum);
                    if(progr1-progr0-0.01>1e-6 || ((ii+1==probNum) & (jj+1==expNum)))
                    {
                        if((ii+1==probNum) & (jj+1==expNum)) progr1=1.0;
                        progress_print(progr1);
                        progr0=progr1;
                    }
                ...
                init_smr_wk(&smrwk);
                smrwk.cur_prbidx=jj;
                fill_smr_wk_trans(&bdata, &gdata, &esdata, &meddata, &smrwk);      

                if (smrwk.bxz.size() == 0) {
                    cout<<"WARNING: no SNP fetched for probe [ " + medname + " ]."<<endl;
                    //printf("WARNING: no SNP fetched for probe [ %s ].\n", exponame.c_str());
                    continue;
                }
                cout<< atos(smrwk.bxz.size()) + " SNPs are included for mediator probe [ " + medname + " ]."<<endl;
                ...
                double pmz_val = 2*pnorm(fabs(zsmz[maxidm]));

                int snp_count=mr_mediation_pruning(&bdata, &smrwk, expochr, medchr, pxz_val, pmz_val, p_smr, p_medsmr, ld_top_multi, multi_corr_snp_flg, multi_exp_priority_flg, get_snp_effects_flg, min_snp, snpfile, snpfilename, exponame, medname);

                if(snp_count==-9){
                    //printf("No mediation analysis performed because less than 3 SNPs passed the p-value or LD-square thresholds. \n");
                    cout<<"No SNPs written down for this exposure-mediator pair because less than "<<min_snp<< " SNPs passed the pruning."<<endl;
                    cout<<logstr<<endl;
                    continue;
                }
        ...
        cout<<"\nExposure to multiple mediators - Harmonizing and Pruning completed.\n Results of "<<write_count<<" mediator probes have been saved in the file [" + snpfilename + "]."<<endl;
